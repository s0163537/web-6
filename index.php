...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE[session_name()]) && !empty($_SESSION['login'])) {
    ...
    $db = new PDO('mysql:host=localhost;dbname=u47495', 'u47495', '9466229', array(PDO::ATTR_PERSISTENT => true));
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    $stmt = $db->prepare("UPDATE people SET name = ?, email = ?, year = ?, gender = ?, limbs = ?, bio = ?");
    ...
    foreach($ability as $item) {
      $stmt = $db->prepare("INSERT INTO superability SET people_id = ?, name_of_superability = ?");
    ...
    $stmt = $db->prepare("INSERT INTO people SET name = ?, email = ?, year = ?, gender = ?, limbs = ?, bio = ?");
    $stmt -> execute([$_POST['name'], $_POST['email'], $_POST['year'], $_POST['gender'], $_POST['limbs'], $_POST['bio']]);
